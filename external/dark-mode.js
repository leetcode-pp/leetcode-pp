  ...
  constructor(options, callback) {
    ...
    if (options.remember) {
      if (localStorage.getItem(options.remember)) {
        ...
        options.onChange && options.onChange(options.default === 'dark')
      }
    ...
    document.addEventListener(
      'darken-dark',
      this.__handleDarkenEvent(options, callback, 'add'),
      false
    )
    ...
    document.addEventListener(
      'darken-light',
      this.__handleDarkenEvent(options, callback, 'remove'),
      false
    )
    ...
    return () => {
      if (options.container)
        document
          ...
          .classList[action](options.class)
      ...
      for (const [key, value] of Object.entries(options.variables)) {
        ...
        if (value && typeof value === 'object') {
          if (Array.isArray(value))
            element.style.setProperty(key, this.dark ? value[1] : value[0])
          else
            element.style.setProperty(key, value[this.dark ? 'dark' : 'light'])
        }
      }
